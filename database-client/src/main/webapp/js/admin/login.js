$(function () {
    ...
    var pageFuns = {
        ...
        bindEvent: function () {
            $("#submit").click(pageFuns.login);
        },

        login: function () {
            var username = $.trim($("#username").val());
            var password = $("#password").val();
            if (!pageFuns.check(username,password)) {
                return;
            }
            $.ajax({
                url : Constants.serverPath + "auth/adminLogin.do",
                data : {
                    "username" : username,
                    "password" : password
                },
                ...
                success : function (resultData) {
                    if(!resultData.success) {
                        var errorMsg = resultData.message;
                        myAlert(null,errorMsg,null);
                        return;
                    }
                    window.location.href = Constants.serverPath + "admin/index.jsp";
                },
                error : function(){
                    window.location.href = Constants.serverPath + "admin/login.jsp";
                }
            });
        },
        ...
        check : function (username,password) {
            if (username == "") {
                myAlert(null,"用户名不可以为空",null);
                return false;
            }
            if (password == "") {
                myAlert(null,"密码不可以为空",null);
                return false;
            }
            return true;
        }
